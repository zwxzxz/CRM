...
/**
 * CategoryController 线索来源类别控制器
 *
 * @author zwx
 */
public class ClueSourceCategoryController implements Initializable {
    /**
     * 获得布局文件中的表格对象
     */
    @FXML
    private TableView<ClueSourceCategory> sourcecategoryTable;

    /**
     * 定义ObservableList数据集合
     */
    private final ObservableList<ClueSourceCategory> sourcecategoryData = FXCollections.observableArrayList();

    /**
     * 通过工厂类获得sourceCategoryDAO的实例
     */
    private final ClueSourceCategoryDAO sourcecategoryDAOClue = DaoFactory.getClueSourceCategoryDAOInstance();
    ...
    private final TableColumn<ClueSourceCategory, ClueSourceCategory> delCol = new TableColumn<>("操作");
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        delCol.setCellFactory(param -> new TableCell<>() {
            ...
            @Override
            protected void updateItem(ClueSourceCategory sourcecategory, boolean empty) {
                ...
                deleteButton.setOnAction(event -> {
                    ...
                    if (result.get() == ButtonType.OK) {
                        ...
                        try {
                            sourcecategoryDAOClue.deleteById(sourcecategory.getId());
        ...
        try {
            entityList = sourcecategoryDAOClue.selectAll();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void addCategory() {
        ...
        if (result.isPresent()) {
            ...
            ClueSourceCategory sourcecategory = new ClueSourceCategory();
            sourcecategory.setSourcename(name);
            long id = 0;
            try {
                id = sourcecategoryDAOClue.insert(sourcecategory);
            } catch (SQLException e) {
                e.printStackTrace();
            }
